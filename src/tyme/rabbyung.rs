...
impl RabByungYear {
  pub fn new(rab_byung_index: isize, element_index: usize, zodiac_index: usize) -> Result<Self, String> {
  ...
  pub fn from_sixty_cycle(rab_byung_index: isize, sixty_cycle: SixtyCycle) -> Result<Self, String> {
    Self::new(rab_byung_index, sixty_cycle.get_heaven_stem().get_element().get_index(), sixty_cycle.get_earth_branch().get_zodiac().get_index())
  }
...
impl RabByungMonth {
  ...
  pub fn validate(year: isize, month: isize) -> Result<(), String> {
    ...
    if year < 1950 || year > 2050 {
      return Err(format!("rab-byung year {} must between 1950 and 2050", year));
    }

    let leap: bool = month < 0;
    let m: usize = month.abs() as usize;
    if year == 1950 && m < 12 {
      return Err(format!("month {} must be 12 in rab-byung year {}", month, year));
    }

    if leap && m != RabByungYear::from_year(year)?.get_leap_month() {
      return Err(format!("illegal leap month {} in rab-byung year {}", m, year));
    }
...
impl Display for RabByungMonth {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    write!(f, "{}{}", self.get_rab_byung_year().to_string(), self.get_name())
  }
}
...
impl Display for RabByungDay {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    write!(f, "{}{}", self.get_rab_byung_month().to_string(), self.get_name())
  }
}
...
mod tests {
  ...
  fn test6() {
    ...
    let y: RabByungYear = RabByungYear::from_element_zodiac(15, RabByungElement::from_name("铁"), Zodiac::from_name("虎")).unwrap();
    assert_eq!("1951年1月8日", RabByungDay::from_ymd(y.get_year(), 12, 1).unwrap().get_solar_day().to_string());
  }

  #[test]
  fn test7() {
    assert_eq!("第十八饶迥铁马年十二月三十", SolarDay::from_ymd(2051, 2, 11).get_rab_byung_day().unwrap().to_string());
    let y: RabByungYear = RabByungYear::from_element_zodiac(17, RabByungElement::from_name("铁"), Zodiac::from_name("马")).unwrap();
    assert_eq!("2051年2月11日", RabByungDay::from_ymd(y.get_year(), 12, 30).unwrap().get_solar_day().to_string());
  }

  #[test]
  fn test8() {
    assert_eq!("第十七饶迥木蛇年二月廿五", SolarDay::from_ymd(2025, 4, 23).get_rab_byung_day().unwrap().to_string());
    let y: RabByungYear = RabByungYear::from_element_zodiac(16, RabByungElement::from_name("木"), Zodiac::from_name("蛇")).unwrap();
    assert_eq!("2025年4月23日", RabByungDay::from_ymd(y.get_year(), 2, 25).unwrap().get_solar_day().to_string());
  }

  #[test]
  fn test9() {
    assert_eq!("第十六饶迥铁兔年正月初二", SolarDay::from_ymd(1951, 2, 8).get_rab_byung_day().unwrap().to_string());
    let y: RabByungYear = RabByungYear::from_element_zodiac(15, RabByungElement::from_name("铁"), Zodiac::from_name("兔")).unwrap();
    assert_eq!("1951年2月8日", RabByungDay::from_ymd(y.get_year(), 1, 2).unwrap().get_solar_day().to_string());
  }
  ...
  fn test10() {
    ...
    let y: RabByungYear = RabByungYear::from_element_zodiac(15, RabByungElement::from_name("铁"), Zodiac::from_name("虎")).unwrap();
    assert_eq!("1951年1月24日", RabByungDay::from_ymd(y.get_year(), 12, -16).unwrap().get_solar_day().to_string());
  }

  #[test]
  fn test11() {
    assert_eq!("第十六饶迥铁牛年五月十一", SolarDay::from_ymd(1961, 6, 24).get_rab_byung_day().unwrap().to_string());
    let y: RabByungYear = RabByungYear::from_element_zodiac(15, RabByungElement::from_name("铁"), Zodiac::from_name("牛")).unwrap();
    assert_eq!("1961年6月24日", RabByungDay::from_ymd(y.get_year(), 5, 11).unwrap().get_solar_day().to_string());
  }
  ...
  fn test12() {
    ...
    let y: RabByungYear = RabByungYear::from_element_zodiac(15, RabByungElement::from_name("铁"), Zodiac::from_name("兔")).unwrap();
    assert_eq!("1952年2月23日", RabByungDay::from_ymd(y.get_year(), 12, 28).unwrap().get_solar_day().to_string());
  }
