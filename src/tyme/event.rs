...
lazy_static! {
  static ref EVENT_MANAGER_CHARS: Vec<char> = "0123456789ABCDEFGHIJKLMNOPQRSTU_VWXYZabcdefghijklmnopqrstuvwxyz".chars().collect();
}
...
impl Event {
  ...
  pub fn get_solar_day(&self, year: isize) -> Option<SolarDay> {
    ...
    let chars: Vec<char> = self.data.chars().collect();
    let offset: isize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[5]).unwrap() as isize - 31;
    if offset != 0 {
  ...
  fn get_solar_day_by_solar_day(&self, year: isize) -> Option<SolarDay> {
    ...
    let chars: Vec<char> = self.data.chars().collect();
    let mut m: usize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[2]).unwrap() - 31;
    if m > 12 {
      m = 1;
      y += 1;
    }
    let d: usize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[3]).unwrap() - 31;
    let delay: isize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[4]).unwrap() as isize - 31;
  ...
  fn get_solar_day_by_lunar_day(&self, year: isize) -> Option<SolarDay> {
    let mut y: isize = year;
    let chars: Vec<char> = self.data.chars().collect();
    let mut m: isize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[2]).unwrap() as isize - 31;
    if m > 12 {
      m = 1;
      y += 1;
    }
    ...
    if d > last_day {
      if 0 == delay {
        ...
        return Some(LunarDay::from_ymd(y, m, (d as isize + delay) as usize).get_solar_day());
      }
      return Some(LunarDay::from_ymd(y, m, last_day).get_solar_day().next(delay));
    }
  ...
  fn get_solar_day_by_week(&self, year: isize) -> Option<SolarDay> {
    let chars: Vec<char> = self.data.chars().collect();
    // 第几个星期
    let n: isize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[3]).unwrap() as isize - 31;
    if n == 0 {
      return None;
    }
    let m: SolarMonth = SolarMonth::from_ym(year, (EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[2]).unwrap() as isize - 31) as usize);
    // 星期几
    let w: isize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[4]).unwrap() as isize - 31;
    if n > 0 {
      // 当月第1天
  ...
  fn get_solar_day_by_term(&self, year: isize) -> Option<SolarDay> {
    let chars: Vec<char> = self.data.chars().collect();
    let offset: isize = EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[4]).unwrap() as isize - 31;
    let d: SolarDay = SolarTerm::from_index(year, EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[2]).unwrap() as isize - 31).get_solar_day();
    if offset != 0 {
  ...
  fn get_solar_day_by_term_heaven_stem(&self, year: isize) -> Option<SolarDay> {
    ...
    let chars: Vec<char> = self.data.chars().collect();
    Some(d.next(d.get_lunar_day().get_sixty_cycle().get_heaven_stem().steps_to(EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[3]).unwrap() as isize - 31) as isize))
  }

  fn get_solar_day_by_term_earth_branch(&self, year: isize) -> Option<SolarDay> {
    let d: SolarDay = self.get_solar_day_by_term(year)?;
    let chars: Vec<char> = self.data.chars().collect();
    Some(d.next(d.get_lunar_day().get_sixty_cycle().get_earth_branch().steps_to(EVENT_MANAGER_CHARS.iter().position(|&c| c == chars[3]).unwrap() as isize - 31) as isize))
  }
}
...
impl EventBuilder {
  ...
  pub fn solar_day(self, solar_month: usize, solar_day: usize, delay_days: isize) -> Self {
    self.content(EventType::SolarDay, solar_month as isize, solar_day as isize, delay_days)
  }

  pub fn lunar_day(self, lunar_month: isize, lunar_day: usize, delay_days: isize) -> Self {
    self.content(EventType::LunarDay, lunar_month, lunar_day as isize, delay_days)
  }

  pub fn solar_week(self, solar_month: usize, week_index: isize, week: usize) -> Self {
    self.content(EventType::SolarWeek, solar_month as isize, week_index, week as isize)
  }
  ...
  pub fn term_heaven_stem(self, term_index: usize, heaven_stem_index: usize, delay_days: isize) -> Self {
    self.term(EventType::TermHs, term_index as isize, heaven_stem_index as isize, delay_days)
  }

  pub fn term_earth_branch(self, term_index: usize, earth_branch_index: usize, delay_days: isize) -> Self {
    self.term(EventType::TermEb, term_index as isize, earth_branch_index as isize, delay_days)
  }
  ...
  pub fn start_year(mut self, year: isize) -> Self {
    ...
    for i in 0..3 {
      self.data[8 - i] = *EVENT_MANAGER_CHARS.get((n % size as isize) as usize).unwrap();
      n /= size as isize;
    }
    self
  }
...
mod tests {
  ...
  fn setup() {
    ...
    EventManager::update("情人节", Event::builder().solar_day(2, 14, 0).start_year(1986).build());
    EventManager::update("国际消费者权益日", Event::builder().solar_day(3, 15, 0).start_year(1983).build());
    EventManager::update("愚人节", Event::builder().solar_day(4, 1, 0).start_year(0).build());
    EventManager::update("万圣夜", Event::builder().solar_day(10, 31, 0).start_year(0).build());
    EventManager::update("万圣节", Event::builder().solar_day(11, 1, 0).start_year(0).build());
    EventManager::update("平安夜", Event::builder().solar_day(12, 24, 0).start_year(0).build());
    EventManager::update("圣诞节", Event::builder().solar_day(12, 25, 0).start_year(0).build());
    ...
    EventManager::update("父亲节", Event::builder().solar_week(6, 3, 0).start_year(1914).build());
    EventManager::update("感恩节", Event::builder().solar_week(11, 4, 4).start_year(1914).build());
