...
impl Taboo {
  ...
  pub fn get_day_recommends(month: SixtyCycle, day: SixtyCycle) -> Vec<Self> {
    let mut l: Vec<Self> = Vec::new();
    let m: Vec<&str> = DAY_TABOO[month.get_earth_branch().get_index()].split(";").collect();
    let d: Vec<&str> = m[day.get_index()].split(",").collect();
    let data: &str = d[0];
    for i in (0..data.len()).step_by(2) {
      let d: &str = &data[i..i + 2];
      l.push(Self::from_index(isize::from_str_radix(d, 16).unwrap()))
  ...
  pub fn get_hour_recommends(day: SixtyCycle, hour: SixtyCycle) -> Vec<Self> {
    let mut l: Vec<Self> = Vec::new();
    let h: Vec<&str> = HOUR_TABOO[hour.get_earth_branch().get_index()].split(";").collect();
    let d: Vec<&str> = h[day.get_index()].split(",").collect();
    let data: &str = d[0];
    for i in (0..data.len()).step_by(2) {
      let d: &str = &data[i..i + 2];
      l.push(Self::from_index(isize::from_str_radix(d, 16).unwrap()))
