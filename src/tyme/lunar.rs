...
pub static LUNAR_MONTH_NAMES: [&str; 12] = [
    "正月",
    "二月",
    "三月",
    "四月",
...
impl LunarMonth {
    ...
    pub fn get_week_count(&self, start: usize) -> usize {
        ((AbstractCulture::new().index_of(
            (self.get_first_julian_day().get_week().get_index() as isize) - (start as isize),
            7,
    ...
    pub fn get_sixty_cycle(&self) -> SixtyCycle {
        ...
        SixtyCycle::from_name(
            format!(
                ...
                .get_name(),
                EarthBranch::from_index(month + 1).get_name()
            )
            .as_str(),
        )
    }
...
impl LunarDay {
    ...
    pub fn is_before(&self, target: LunarDay) -> bool {
        ...
        if a_month != b_month {
            if a_month.abs() != b_month.abs() {
                return a_month.abs() < b_month.abs();
            }
            // 同月号，正月在闰月之前（正数为正月，负数为闰月）
            return a_month > 0;
        }
